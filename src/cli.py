...
def manage_finances(db: Session, user: User):
    ...
    while True:
        click.echo("\nOptions: [1] Manage Transactions [2] Manage Budgets [3] Analyze Spending [4] Manage Recurring Transactions [5] Exit")
        ...
        if trans_choice == 1:
            ...
            if transactions:
                for txn in transactions:
                    click.echo(f'{txn.date}: {txn.category} - ${txn.amount}')
            else:
        ...
        if budget_choice == 1:
            ...
            if budgets:
                for budget in budgets:
                    click.echo(f'{budget.id}: {budget.category} - ${budget.limit}')
            else:
            ...
            if budgets:
                ...
                if updated_budget:
                    click.echo(f'Budget updated: {updated_budget.category} - ${updated_budget.limit}')
                else:
        ...
        if sub_choice == 1:
            ...
            if recurring_transactions:
                for rtxn in recurring_transactions:
                    click.echo(f'{rtxn.id}: {rtxn.category} - ${rtxn.amount} (Next: {rtxn.next_occurrence})')
            else:
